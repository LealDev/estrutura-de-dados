...
public class LCInteiroNovo {
	...
	public LCInteiroNovo(int tamanho) {
	...
	// Exercicio 05
	public void divide (LCInteiroNovo l1, LCInteiroNovo l2) {
		int tamanho = this.getQuant()/2;
		int i, j;
		LCInteiroNovo aux = l1;
		l1 = new LCInteiroNovo(tamanho);
		l2 = new LCInteiroNovo(aux.getQuant() - tamanho);
		for (i=0; i<tamanho; i++) {
			l1.insereFim(this.getNum(i));
		}
		for (j=i; j<aux.getQuant();j++) {
			l2.insereFim(aux.getNum(j));
		}
	}
}
